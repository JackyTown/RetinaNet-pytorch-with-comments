...
class RetinaNet(nn.Module):
    ...
    def __init__(self, num_classes=20):
        ...
        self.loc_head = self._make_head(self.num_anchors*4) # 输出通道为self.num_anchors*4
        self.cls_head = self._make_head(self.num_anchors*self.num_classes) 
        # 输出通道为self.num_anchors*self.num_classes
    ...
    def forward(self, x):
        ...
        return torch.cat(loc_preds,1), torch.cat(cls_preds,1) # [N, len(fms)*9*4, ...] [N, len(fms)*9*20, ...]

    def _make_head(self, out_planes): #特征解析网络,输出loc & cls
        layers = []
        for _ in range(4): # 4层
